...
```
결과는 아래 그림의 왼쪽에 나타냈다. 오른쪽 그림은 10차 다항식 커널을 사용한 또 다른 SVM ㅜㄴ류기이다. 모델이 과대적합이라면 다항식의 차수를 줄여야 한다. 반대로 과소적합이라면 차수를 늘려야 한다. 매개변수 coef0는 모델이 높은 차수와 낮은 차수에 얼마나 영향을 받을지 조절한다.
